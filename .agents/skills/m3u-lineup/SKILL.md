...
5. Do not invent metadata: preserve `tvg-name`, `tvg-logo`, `group-title`, `audio-track`, `user-agent`, and `#EXTVLCOPT` when they exist. Do not add `tvg-id` (see `.agents/AGENTS.md`). Never convert Astra MPEG-TS `/play/<id>` into `/index.m3u8`.
